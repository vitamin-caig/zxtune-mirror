...
class RemoteCatalogTest {
    ...
    fun `test groups`() {
        ...
        val result = ArrayList<Group>().apply {
            catalog.queryGroups(object : Catalog.GroupsVisitor() {
                override fun accept(obj: Group) {
                    add(obj)
                }
            })
        }
        check(8823, checkpoints, result)
    }
    ...
    fun `test authors tracks`() {
        ...
        val result = ArrayList<Track>().apply {
            catalog.queryTracks(
                Author(2085, "doh", "Nicolas Dessesart"),
                object : Catalog.TracksVisitor() {
    ...
    fun `test search`() {
        ...
        catalog.findTracks("zzz", object : Catalog.FoundTracksVisitor() {
...
private fun <T> check(expectedSize: Int, expected: Array<T>, actual: ArrayList<T>) {

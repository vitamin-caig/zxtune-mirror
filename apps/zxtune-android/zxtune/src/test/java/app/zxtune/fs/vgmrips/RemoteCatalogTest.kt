...
class RemoteCatalogTest {
    ...
    fun `test chips`() {
        val checkpoints = arrayOf( //first
            Group("nes-apu", "NES APU", 436),  //last
            Group("vrc7", "VRC7", 1)
        )
        with(GroupsChecker()) {
            catalog.chips().query(this)
            check(49, checkpoints)
        }
    }
    ...
    fun `test pack tracks`() {
        val checkpoints = arrayOf( //first
            Track(
                1, "Into The Lair", TimeStamp.createFrom(
                    54,
                    TimeUnit.SECONDS
                ), "Other/The_Scheme_(NEC_PC-8801,_OPNA)/01 Into The Lair.vgz"
            ),  //last
            Track(
                17, "Theme of Gigaikotsu", TimeStamp.createFrom(
                    121,
                    TimeUnit.SECONDS
...
private fun <T> check(expectedSize: Int, expectedSubset: Array<T>, actual: ArrayList<T>) {
    assertEquals(expectedSize.toLong(), actual.size.toLong())
...
private fun matches(expected: Group, actual: Group) =
    if (expected.id == actual.id) {
        ...
        assertEquals(expected.id, expected.packs.toLong(), actual.packs.toLong())
        true
    } else {
        false
    }

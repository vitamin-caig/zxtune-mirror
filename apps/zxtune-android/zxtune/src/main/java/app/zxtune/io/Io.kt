...
object Io {
    ...
    private fun reallocate(buf: ByteArray?): ByteArray {
        ...
        while (true) {
            try {
                return if (buf != null) {
                    val result = ByteArray(buf.size * 3 / 2)
                    System.arraycopy(buf, 0, result, 0, buf.size)
                    result
                } else {
    ...
    @JvmStatic
    fun rename(oldName: File, newName: File): Boolean {
    ...
    @RequiresApi(26)
    private fun renameViaFiles(oldName: File, newName: File): Boolean {
        try {
            Files.move(
                oldName.toPath(),
                newName.toPath(),
    ...
    @JvmStatic
    fun createByteBufferInputStream(buf: ByteBuffer): InputStream {

...
class RemoteCatalog(val http: MultisourceHttpProvider) : Catalog {
    ...
    fun readDoc(uri: Uri): Document = HtmlUtils.parseDoc(http.getInputStream(uri))
    ...
    doc.select("table.playlist>tbody>tr:has(>td.title)")
        .forEach { track ->
            ...
            if (number > 0 && !title.isNullOrEmpty() && location.isNotEmpty()) {
                val duration = TimeStamp.createFrom(
                    tryParseIntPrefix(track.attr("data-duration")).toLong(),
                    TimeUnit.SECONDS
                )
